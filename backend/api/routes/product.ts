  ...
  app.get('/api/v1/seller/products', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const payload = await withAdmin(baseDb, user.id, async (db) => {
        ...
        let base = db('products').where({ store_id: store.id });
        if (status) base = base.andWhere('status', status);
        if (search) {
          base = base.andWhere((qb: any) => {
            qb.whereILike('title', `%${search}%`).orWhereILike('sku', `%${search}%`);
          });
        }
        ...
        return { code: 200 as const, body: { ok: true, items, pagination: { page, limit, total, pages: Math.ceil(total / limit) } } };
      });
