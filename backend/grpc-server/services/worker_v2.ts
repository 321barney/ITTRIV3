...
export const IngestServiceImpl = {
  async Health(call: any, callback: any) {
    try {
      ...
      logger.info({ method: "IngestService.Health.v2", rid });
      const resp = { status: "SERVING", version: "v0.2.0" };
      callback(null, resp);
    } catch (e) {
  ...
  async Warm(call: any, callback: any) {
    try {
      ...
      logger.info({ method: "IngestService.Warm.v2", subject });
      callback(null, {});
    } catch (e) {
      callback(e as Error, null as any);
    }
  ...
  async KickIngest(call: any, callback: any) {
    try {
      ...
      logger.info({ method: "IngestService.KickIngest.v2", subject, jobId, label: req.label });
  ...
  async Upload(call: any, callback: any) {
    try {
      ...
      logger.info({ method: "IngestService.Upload.v2", subject, fileName, size });
...
export const ConversationServiceImpl = {
  ...
  async Kick(call: any, callback: any) {
    try {
      ...
      const req = call.request;
      if (!req.label) throw ERR.invalidArgument("label is required");
      logger.info({ method: "ConversationService.Kick.v2", subject, label: req.label });
