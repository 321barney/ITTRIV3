...
export async function ensureSession(
  db: Knex,
  sellerId: string,
  opts?: { storeId?: string | null; title?: string | null; sessionId?: string | null; fileId?: string | null }
): Promise<string> {
  const { sessionId, storeId = null, title = null, fileId = null } = opts || {};
  ...
  // If fileId is provided, try to find or create a session for that file
  if (fileId) {
    const existingFileSession = await db('ai_chat_sessions')
      .where({ seller_id: sellerId })
      .whereRaw(`${metaCol}->>'file_id' = ?`, [fileId])
      .orderBy('created_at', 'desc')
      .first('id');
    
    if ((existingFileSession as any)?.id) {
      return (existingFileSession as any).id as string;
    }
  } else {
    // reuse most recent; prefer 'open' when available (only if no fileId)
  ...
  const insertPayload: any = {
    seller_id: sellerId,
    store_id: storeId,
    title: title || (fileId ? `Chat: ${fileId.slice(0, 8)}` : null),
  };
  insertPayload[metaCol] = fileId ? { file_id: fileId } : {};
