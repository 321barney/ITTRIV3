...
-- Remove legacy multi-integration leftovers (if any)
...
BEGIN
  FOREACH t IN ARRAY tables LOOP
    ...
    EXECUTE format(
      'CREATE POLICY %1$I_policy ON %1$I FOR ALL TO app_user USING (seller_id = app.current_seller_id()) WITH CHECK (seller_id = app.current_seller_id())',
      t
    );
  END LOOP;
END;
...
BEGIN
  ...
  LOOP
    BEGIN
      EXECUTE format('GRANT SELECT,INSERT,UPDATE,DELETE ON %I TO app_user, app_admin', t);
