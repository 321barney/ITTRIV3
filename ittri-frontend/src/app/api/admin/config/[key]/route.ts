...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const upstreamUrl = backendUrl(`/admin/config/${id}`);
    ...
    const up = await fetch(upstreamUrl, { method: "PUT", headers: { ...pass, cookie: cookies }, body, signal: ctrl.signal, cache: "no-store", redirect: "manual" });
    const text = await up.text();
    const res = new NextResponse(text, { status: up.status, headers: filteredResponseHeaders(up) });
    res.headers.set("x-which-route", "admin-config-key");
